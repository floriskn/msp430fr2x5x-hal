...
static CAPTURE: Mutex<UnsafeCell<Option<Capture<msp430fr413x::Ta2, CCR1>>>> =
    Mutex::new(UnsafeCell::new(None));
static VECTOR: Mutex<UnsafeCell<Option<TBxIV<msp430fr413x::Ta2, DefaultMapping>>>> =
    Mutex::new(UnsafeCell::new(None));
...
#[entry]
fn main() -> ! {
    let Some(periph) = msp430fr413x::Peripherals::take() else { loop {} };
